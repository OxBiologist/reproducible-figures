...
```{r cleaning data etc, echo = FALSE, message = FALSE, waning = FALSE, include=FALSE}
#load packages:
library(ggplot2)
...
culmen_data <- penguins_clean %>%
    select(c("culmen_length_mm", "culmen_depth_mm", "species","sex")) %>%
    na.omit()
```
...
bad_plot <- ggplot(culmen_data, aes(x = culmen_length_mm, y = culmen_depth_mm)) +
  geom_point(aes(color = species), size = 3, alpha = 0.5) +
  geom_smooth(method = lm, color = "#D21404", size = 2, se = FALSE) +
  labs(x = "X", y = "Y") +
  coord_cartesian(xlim = c(20, 60), ylim = c(13, 60)) +
  scale_x_log10() +
  scale_color_manual(values = c("#D0312D", "#E3242B", "#B90E0A")) +
...
This graph, aiming to investigate the correlation between culminate length and depth in the Palmer Penguin data set, has numerous issues that could be misleading. Firstly, there is no title or axis labels leaving the reader clueless as to what the graph is investigating. The lack of legend also means the point colour coding cannot be understood (here it relates to penguin species). On the subject of which, even if there were a legend, the choice of colours would make it near impossible to determine which species a point belongs to as all three colours are incredibly similar. Further, the regression line itself is also red making it a challenge to distinguish (especially for those with colourblindness who struggle to differentiate colours). The colour coding issues are worsened by the translucency of the points, meaning where they overlap the colour changes. Such overlapping points are commonplace here due to the unnecessarily large point size, which also makes reading exact values impossible. In reality, when plotted correctly, this data shows little correlation, with the regression line being drawn through a large cloud of data with little significance. However, the drawing of this graph clusters all the data together in the bottom corner, making the correlation (with the regression line) seem much more significant than reality. This bunching up of the data is caused by axes being much longer than necessary (especially the y axis), despite neither starting at zero. The condensing of the points is also worsened by the x axis being logarithmic. Finally, there is no indication of confidence interval around the regression line, so making the correlation seem more certain than is the case.
